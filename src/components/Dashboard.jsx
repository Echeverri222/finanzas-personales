...
import { importarMovimientos } from '../utils/importMovimientos';
...
export default function Dashboard({ onQuickMovement }) {
  ...
  const handleImportarMovimientos = async () => {
    if (!userProfile) return;

    try {
      setImportLoading(true);
      const result = await importarMovimientos(userProfile.id);
      ...
      alert('Error al importar los movimientos: ' + err.message);
    } finally {
    ...
    <div className="p-2 md:p-6 space-y-4 md:space-y-6 bg-gray-50">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-2 md:gap-4">
          <button
            onClick={handleImportarMovimientos}
            disabled={importLoading}
            className={`px-4 py-2 rounded-lg text-white transition-colors ${
              importLoading ? 'bg-gray-400' : 'bg-purple-500 hover:bg-purple-600'
            }`}
          >
            {importLoading ? (
